...
Client.prototype.connect = function (cfg) {
      ...
      instance
        ...
        .then(function (response) {
          ...
          self._rest_config = {
            baseURL: self.config.baseURL,
            timeout: self.config.timeout,
            headers: { "orbit-session-token": self._token },
          };
  ...
  const doAccept = (response) => {
    if (self.config.debug)
      console.log(
        `${ts()} - token: ` + self._token + " My UserID: " + self._user_id
      );
  ...
  ost.then(doAccept).catch(doReject);
};
...
Client.prototype.devices = function () {
  ...
  const getDevices = () => {
    return new Promise((resolve, reject) => {
      ...
      instance
        .get("/v1/devices?user_id=" + self._user_id)
        .then(function (response) {
          if (self.config.debug)
            console.log(
              `${ts()} - response.data: ` + JSON.stringify(response.data)
  ...
  const doAccept = (response) => {
    if (self.config.debug)
      console.log(`${ts()} - response.data: ` + JSON.stringify(response.data));
  ...
  Devices.then(doAccept).catch(doReject);
};
...
Client.prototype.connectStream = function () {
  ...
  const authenticate = () => {
    ...
    if (self.config.debug)
      console.log(
        `${ts()} - websocket authenticate message: ` + JSON.stringify(message)
      );
  ...
  self._stream.on("unexpected-response", function (request, response) {
    console.error(
      `${ts()} - unexpected-response / request: ` +
        request +
        " response: " +
        response
    );
